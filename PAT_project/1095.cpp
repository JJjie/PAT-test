...
bool cmp_951(node95 a, node95 b) {
    if(strcmp(a.id, b.id) != 0)
        return strcmp(a.id, b.id) < 0;
    else
        return a.time < b.time;
}
bool cmp_952(node95 a, node95 b) {
    return a.time < b.time;
}
int pat_1095() {
    int n, k, maxtime = -1, tempindex = 0;
    scanf("%d%d\n", &n, &k);
    vector<node95> record(n), car;
    for(int i = 0; i < n; i++) {
        char temp[5];
        int h, m, s;
    ...
    sort(record.begin(), record.end(), cmp_951);
    map<string, int> mapp;
    for(int i = 0; i < n - 1; i++) {
    ...
    sort(car.begin(), car.end(), cmp_952);
    vector<int> cnt(n);
