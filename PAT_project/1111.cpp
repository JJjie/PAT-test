...
int dis[510], Time[510], e[510][510], w[510][510], dispre[510], weight[510];//记录等长的路时间最少的消耗
bool visit111[510];
vector<int> dispath, Timepath, Temppath, Timepre[510];//存多条时间相同的路
...
int pat_1111() {
    ...
    dfsdispath(fin);
    Time[st] = 0;
    fill(visit111, visit111 + 510, false);
    for(int i = 0; i < n; i++) {
        int u = -1, minn = inf;
        for(int j = 0; j < n; j++) {
            if(!visit111[j] && minn > Time[j]) {
                u = j;
                minn = Time[j];
            }
        }
